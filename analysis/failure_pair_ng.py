...
def latents(cfg, hydra_cfg):
    ...
    orbit_paths = orbit_set.dataset['path']

    # Calculate pairwise distances in the 2D UMAP space
    distances = pdist(emb_orbit, 'cosine')
    ...
    inr_a_idx = max_dist_idx[0]
    inr_b_idx = max_dist_idx[1]

    # Get the file paths for this pair
    inr_a_path = orbit_paths[inr_a_idx]
    inr_b_path = orbit_paths[inr_b_idx]

    print("="*50)
    print("Found most distant pair for Neural Graphs:")
    print(f"  INR A: {inr_a_path}")
    print(f"  INR B: {inr_b_path}")
    print(f"  Distance in UMAP space: {distance_matrix[max_dist_idx]:.4f}")
    print("="*50)
# ────────────────────────────────────────────────────────────
