...
class NFNZooDataset(CNNDataset):
    ...
    def __init__(
        ...
        # assert interpolation and debug are mutually exclusive
        assert not (interpolation and debug), "Cannot use interpolation and debug at the same time."
        if interpolation:
            idxs = random.sample(range(len(data)), 2)  # choose 2 unique indices
            data = data[idxs]
            self.metrics = self.metrics.iloc[idxs]
        if interpolation_many_pairs:
            sorted_indices = self.metrics["test_accuracy"].argsort()
            idxs = sorted_indices.iloc[-1500:]
            data = data[idxs]
            self.metrics = self.metrics.iloc[idxs]
        if debug:
            data = data[:1]
    ...
    def __getitem__(self, idx):
        ...
        nfn_data = CNNBatch(weights=weights, biases=biases, y=score)
