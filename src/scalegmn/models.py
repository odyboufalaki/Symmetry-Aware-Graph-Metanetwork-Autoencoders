...
from src.scalegmn.layers import (
    DeepSet,
    PermScaleInvariantReadout,
    MLPNet,
    PositionalEncoding,
    EquivariantNet,
    EdgeUpdate,
)
from src.scalegmn.gnn_layers import (
    ScaleEq_GNN_layer,
    GNN_layer,
    ScaleGMN_GNN_layer_aggr,
    GNN_layer_aggr,
)
...
class BaseScaleGMN(nn.Module):
    def __init__(self, model_args, **kwargs):
        ...
        self.layer_idx = torch.cumsum(
            torch.tensor([0] + model_args["layer_layout"]), dim=0
        )
        ...
        self.model = {"forward": ScaleGMN_GNN_fw, "bidirectional": ScaleGMN_GNN_bidir}

        # if convolution
        if "_max_kernel_height" in model_args:
            model_args["graph_init"]["d_in_e"] = (
                model_args["_max_kernel_height"] * model_args["_max_kernel_width"]
            )

        self.construct_graph = GraphInit(**model_args["graph_init"])

        if self.node_pos_embed:
            self.node2type = get_node_types(self.nodes_per_layer)
            self.positional_embeddings = PositionalEncoding(
                **self.set_pe_args(model_args, "node")
            )

        if self.edge_use_pos_embed:
            self.edge2type = get_edge_types(self.nodes_per_layer)
            self.positional_embeddings_edge = PositionalEncoding(
                **self.set_pe_args(model_args, "edge")
            )
    ...
    def set_pe_args(self, model_args, param) -> dict:
        pe_args = {
            "d_in": (
                model_args["graph_init"][f"d_{param}"]
                if model_args["graph_init"][f"project_{param}_feats"]
                else model_args["graph_init"]["d_in"]
            ),
            ...
            **model_args["positional_encodings"],
            ...
            "ablate_symmetry": model_args["gnn_args"]["ablate_symmetry"],
        }
        return pe_args
...
class ScaleGMN_GNN(nn.Module):
    ...
    def __init__(self, model_args, layer_idx):
        ...
        self.gnn_layer = {
            "forward": (
                ScaleEq_GNN_layer if self.symmetry != "permutation" else GNN_layer
            ),
            "bidirectional": (
                ScaleGMN_GNN_layer_aggr
        ...
        for i in range(model_args["num_layers"]):
            ...
            model_args["gnn_args"]["update_edge_attr"] = (
                model_args["gnn_args"]["update_edge_attr"]
                and i < model_args["num_layers"]
            )
        ...
        if self.gnn_skip_connections:
            n_skips = (
                ...
                if not self.equivariant
                else model_args["num_layers"] - 1
            )
            self.skip_layers = nn.ModuleList(
                [nn.Linear(self.d_hid, self.d_hid, bias=False) for _ in range(n_skips)]
            )
        ...
        if not self.equivariant:
            ...
            d_in_map = {
                ...
                "bidirectional": {True: 2 * self.d_hid, False: self.d_hid},
            }
...
class ScaleGMN_GNN_fw(ScaleGMN_GNN):
    ...
    def forward(self, batch, num_nodes=None, pos_embed=None, pos_embed_edge=None):
        for i, layer in enumerate(self.fw_layers):
            ...
            x_tilde, edge_attr = layer(
                x=x,
                edge_index=batch.edge_index,
                edge_attr=edge_attr,
                mask_hidden=(
                    batch.mask_hidden if hasattr(batch, "mask_hidden") else None
                ),
                mask_first_layer=(
                    batch.mask_first_layer
                    if hasattr(batch, "mask_first_layer")
                    else None
                ),
                pos_embed=pos_embed if pos_embed is not None else None,
                sign_mask=(
                    batch.sign_mask[batch.batch].unsqueeze(-1)
                    if hasattr(batch, "sign_mask")
                    else None
                ),
            )
        ...
        if self.equivariant:
            ...
            if not self.only_last_layer:
                ...
                node_features = x.reshape(
                    batch.num_graphs, num_nodes, x.shape[-1]
                )  # change if processing varying architectures
                node_features = node_features[:, -self.last_layer_nodes :].flatten(1, 2)
                if pos_embed is not None:
                    pos_embed = pos_embed.reshape(
                        batch.num_graphs, num_nodes, pos_embed.shape[-1]
                    )
...
class ScaleGMN_GNN_bidir(ScaleGMN_GNN):
    ...
    def __init__(self, model_args, layer_idx):
        ...
        self.concat_mlp_directions = (
            model_args["concat_mlp_directions"]
            if "concat_mlp_directions" in model_args
            else False
        )

        self.pos_embed_upd = model_args["gnn_args"]["pos_embed_upd"]
        self.update_edge_attr = model_args["gnn_args"]["update_edge_attr"]
        self.reciprocal = model_args["reciprocal"] and not (
            model_args["symmetry"] == "permutation"
        )
        ...
        for i in range(model_args["num_layers"]):
            ...
            if model_args["symmetry"] != "permutation":
                upd_layers.append(
                    EquivariantNet(
                        ...
                        layer_equiv_on_hidden=model_args["gnn_args"][
                            "layer_upd_equiv_on_hidden"
                        ],
                        mlp_on_io=model_args["gnn_args"]["mlp_on_io"],
                        num_mlps=model_args["gnn_args"]["upd_num_mlps"],
                        skip_connections=model_args["gnn_args"][
                            "update_node_feats_fn_skip_connections"
                        ],
                        sign_symmetrization=model_args["gnn_args"][
                            "sign_symmetrization"
                        ],
                        ablate_symmetry=model_args["gnn_args"]["ablate_symmetry"],
                    )
            ...
            if self.update_edge_attr and (
                i < model_args["num_layers"] - 1 or self.equivariant
            ):
                if model_args["symmetry"] != "permutation":
                    fw_upd_edge_layers.append(
                        EdgeUpdate(
                            d_in_v,
                    ...
                    if model_args["symmetry"] != "sign":
                        bw_upd_edge_layers.append(
                            EdgeUpdate(
                                ...
                                ablate_symmetry=model_args["gnn_args"][
                                    "ablate_symmetry"
                                ],
                            )
                        )
        ...
        if not self.equivariant:
            if self.only_last_layer:
                if self.concat_mlp_directions:
                    self.project_first = mlp(
                        in_features=self.first_layer_nodes
                        * model_args["gnn_args"]["d_hid_v"],
                        out_features=model_args["gnn_args"]["d_hid_v"],
                        **model_args["mlp_args"],
                    )
                    self.project_last = mlp(
                        in_features=self.last_layer_nodes
                        * model_args["gnn_args"]["d_hid_v"],
                        out_features=model_args["gnn_args"]["d_hid_v"],
                        **model_args["mlp_args"],
                    )
                else:
                    self.project_first = nn.Linear(
                        self.first_layer_nodes * self.d_hid, self.d_hid
                    )
                    self.project_last = nn.Linear(
                        self.last_layer_nodes * self.d_hid, self.d_hid
                    )
    ...
    def forward(self, batch, num_nodes=None, pos_embed=None, pos_embed_edge=None):
        for i in range(len(self.fw_layers)):
            ...
            if self.update_edge_attr and i < len(self.fw_update_edge_attr_fn):
                edge_attr = self.fw_update_edge_attr_fn[i](
                    x,
                    batch.edge_index,
                    edge_attr,
                    sign_mask=(
                ...
                if self.symmetry == "scale":
                    bw_edge_attr = self.bw_update_edge_attr_fn[i](
                        x,
                        batch.bw_edge_index,
                        bw_edge_attr,
                        sign_mask=(
            ...
            apply_skip = self.gnn_skip_connections and i < len(self.skip_layers)
            x = (
                self.skip_layers[i](x) + x_tilde if apply_skip else x_tilde
            )  # TODO: check if this is correct in every scenario
        ...
        if self.equivariant:
            ...
            if not self.only_last_layer:
                ...
                last_layer_node_features = self.project_last(
                    node_features[:, -self.last_layer_nodes :].flatten(1, 2)
                )
                first_layer_node_features = self.project_first(
                    node_features[:, : self.first_layer_nodes].flatten(1, 2)
                )
                node_features = torch.cat(
                    [last_layer_node_features, first_layer_node_features], dim=1
                )
