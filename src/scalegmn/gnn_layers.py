...
class base_GNN_layer(MessagePassing):
    def __init__(
        ...
        **kwargs,
        ...
        if (
            ...
            and (not last_layer or self.equivariant_gnn)
            ...
            self.update_edge_attr_fn = EdgeUpdate(
                ...
                ablate_symmetry=kwargs["ablate_symmetry"],
            )
    ...
    def forward(
        ...
        if self.update_as_act:
            update_arg = (
                ...
                else self.w_h(x) + self.w_msg(aggregated)
            )
        ...
        if self.update_edge_attr and (not self.last_layer or self.equivariant_gnn):
            out_e = self.update_edge_attr_fn(
                x, edge_index, edge_attr, sign_mask=sign_mask
            )
        else:
            out_e = edge_attr
        return out_v, out_e
    ...
    def __repr__(self):
        ...
        if self.direction == "forward":
            repr_str += (
                f"update_node_feats_fn={self.update_node_feats_fn},\n"
...
class ScaleEq_GNN_layer(base_GNN_layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.update_as_act:
            ...
            self.update_node_feats_fn = SineUpdate(
                sine_input_dim, d_hid
    ...
    def message(
        ...
        msg_j = self.message_fn(
            ...
            mask_first_layer=(
                mask_first_layer[edge_index[1]]
                if mask_first_layer is not None
                else None
            ),
...
class GNN_layer_aggr(base_GNN_layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.message_fn = MLPNet(2 * d_hid, d_hid, kwargs["mlp_args"])
