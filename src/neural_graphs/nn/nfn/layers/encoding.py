...
class IOSinusoidalEncoding(nn.Module):
    def __init__(self, layer_layout, max_freq=10, num_bands=6, enc_layers=True):
        super().__init__()
        self.layer_layout = layer_layout
        self.max_freq = max_freq
        self.num_bands = num_bands
        self.enc_layers = enc_layers
        self.n_in, self.n_out = layer_layout[0], layer_layout[-1]

    def forward(self, wsfeat: WeightSpaceFeatures):
        device, dtype = wsfeat.weights[0].device, wsfeat.weights[0].dtype
        L = len(self.layer_layout) - 1
        layernum = torch.linspace(-1.0, 1.0, steps=L, device=device, dtype=dtype)
        if self.enc_layers:
            layer_enc = fourier_encode(
                layernum, self.max_freq, self.num_bands
            )  # (L, 2 * num_bands + 1)
        else:
            layer_enc = torch.zeros(
                (L, 2 * self.num_bands + 1), device=device, dtype=dtype
            )
        inpnum = torch.linspace(-1.0, 1.0, steps=self.n_in, device=device, dtype=dtype)
        inp_enc = fourier_encode(
            inpnum, self.max_freq, self.num_bands
        )  # (n_in, 2 * num_bands + 1)
        outnum = torch.linspace(-1.0, 1.0, steps=self.n_out, device=device, dtype=dtype)
        out_enc = fourier_encode(
            outnum, self.max_freq, self.num_bands
        )  # (n_out, 2 * num_bands + 1)
        ...
        for i, (weight, bias) in enumerate(wsfeat):
            ...
            bias_enc_i = layer_enc[i][None, :, None].expand(
                b, d, bias.shape[-1]
            )  # (B, d, n_row)
            if i == 0:
                # weight has shape (B, c_in, n_out, n_in)
                inp_enc_i = (
                    inp_enc.transpose(0, 1).unsqueeze(0).unsqueeze(-2)
                )  # (1, d, 1, n_col)
                for _ in axes[2:]:
            ...
            if i == len(wsfeat) - 1:
                out_enc_i = (
                    out_enc.transpose(0, 1).unsqueeze(0).unsqueeze(-1)
                )  # (1, d, n_row, 1)
                for _ in axes[2:]:
