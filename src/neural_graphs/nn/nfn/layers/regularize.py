...
from src.neural_graphs.nn.nfn.layers.layer_utils import shape_wsfeat_symmetry, unshape_wsfeat_symmetry
...
class SimpleLayerNorm(nn.Module):
    def __init__(self, network_spec, channels):
        ...
        for i in range(len(network_spec)):
            eff_channels = int(
                channels * np.prod(network_spec.weight_spec[i].shape[2:])
            )
            self.add_module(
                f"norm{i}_w", nn.LayerNorm(normalized_shape=(eff_channels,))
            )
