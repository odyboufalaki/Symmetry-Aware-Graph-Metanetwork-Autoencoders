...
from src.neural_graphs.nn.nfn.layers.layer_utils import shape_wsfeat_symmetry
...
class UnflattenWeights(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> WeightSpaceFeatures:
        ...
        curr_idx = 0
        for weight_spec, bias_spec in zip(
            self.network_spec.weight_spec, self.network_spec.bias_spec
        ):
            num_wts = np.prod(weight_spec.shape)
            # reshape to (bs, 1, *weight_spec.shape) where 1 is channels.
            wt = (
                x[:, curr_idx : curr_idx + num_wts]
                .view(-1, *weight_spec.shape)
                .unsqueeze(1)
            )
            ...
            bs = (
                x[:, curr_idx : curr_idx + num_bs]
                .view(-1, *bias_spec.shape)
                .unsqueeze(1)
            )
...
class LearnedScale(nn.Module):
    def __init__(self, layer_layout, init_scale):
        super().__init__()
