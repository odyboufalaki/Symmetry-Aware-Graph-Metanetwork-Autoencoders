...
from src.neural_graphs.nn.nfn.common import NetworkSpec, WeightSpaceFeatures
from src.neural_graphs.nn.nfn.layers.layer_utils import (
    set_init_,
...
class Pointwise(nn.Module):
    ...
    def __init__(self, network_spec: NetworkSpec, in_channels, out_channels):
        ...
        for i in range(len(network_spec)):
            fac_i = filter_facs[i]
            self.add_module(
                f"layer_{i}", nn.Conv2d(fac_i * in_channels, fac_i * out_channels, 1)
            )
...
class NPLinear(nn.Module):
    ...
    def __init__(self, layer_layout, in_channels, out_channels, io_embed=False):
        super().__init__()
        self.c_in, self.c_out = in_channels, out_channels
        self.layer_layout = layer_layout
        L = len(layer_layout) - 1
        self.weight_shapes = [
            (layer_layout[i + 1], layer_layout[i]) for i in range(len(layer_layout) - 1)
        ]
        filter_facs = [1] * (
            len(layer_layout) - 1
        )  # [int(np.prod([layer_layout[i+1], layer_layout[i]])) for i in range(len(layer_layout)-1)]
        ...
        for i in range(L):
            ...
            self.add_module(
                f"layer_{i}_rc", nn.Linear(n_rc_inp * in_channels, fac_i * out_channels)
            )
            ...
            self.add_module(
                f"bias_{i}_rc", nn.Linear(n_rc_inp * in_channels, out_channels)
            )
            set_init_(
        ...
        if io_embed:
            ...
            n_in, n_out = layer_layout[0], layer_layout[-1]
            self.in_embed = nn.Parameter(
                torch.randn(1, filter_facs[0] * in_channels, 1, n_in)
            )
            self.out_embed = nn.Parameter(
                torch.randn(1, filter_facs[-1] * in_channels, n_out, 1)
            )
    ...
    def forward(self, wsfeat: WeightSpaceFeatures) -> WeightSpaceFeatures:
        wsfeat = shape_wsfeat_symmetry(wsfeat, self.weight_shapes)
        if self.io_embed:
            new_weights = (
                wsfeat.weights[0] + self.in_embed,
                *wsfeat.weights[1:-1],
                wsfeat.weights[-1] + self.out_embed,
            )
        ...
        for i, (weight, bias) in enumerate(wsfeat):
            ...
            z3 = getattr(self, f"layer_{i}_r")(torch.cat(row_bdcst, dim=-2)).unsqueeze(
                -2
            )  # (B, C_out, 1, ncol)
        ...
        return unshape_wsfeat_symmetry(
            WeightSpaceFeatures(out_weights, out_biases), self.weight_shapes
        )
...
class HNPLinear(nn.Module):
    def __init__(self, network_spec: NetworkSpec, in_channels, out_channels):
        ...
        for i in range(self.L):
            ...
            if i == 0:
                ...
                self.w0_rpt = nn.Conv1d(
                    rpt_in * in_channels, n_in * fac_i * out_channels, 1
                )
                self.w0_rbdcst = nn.Linear(
                    n_rc_inp * in_channels, n_in * fac_i * out_channels
                )
                ...
                if i - 1 == 0:
                    cpt_in += n_in * fac_im1
                self.wfin_cpt = nn.Conv1d(
                    cpt_in * in_channels, n_out * fac_i * out_channels, 1
                )
                self.wfin_cbdcst = nn.Linear(
                    n_rc_inp * in_channels, n_out * fac_i * out_channels
                )
                set_init_(self.wfin_cpt, self.wfin_cbdcst)

                self.bias_fin_rc = nn.Linear(
                    n_rc_inp * in_channels, n_out * out_channels
                )
            else:
                self.add_module(
                    f"layer_{i}",
                ...
                self.add_module(
                    f"layer_{i}_r", nn.Conv1d(row_in, fac_i * out_channels, 1)
                )
                self.add_module(
                    f"layer_{i}_c", nn.Conv1d(col_in, fac_i * out_channels, 1)
                )
                set_init_(
                    getattr(self, f"layer_{i}"),
                    getattr(self, f"layer_{i}_rc"),
    ...
    def forward(self, wsfeat: WeightSpaceFeatures):
        ...
        weights, biases = wsfeat.weights, wsfeat.biases
        col_means = [
            w.mean(dim=-1) for w in weights
        ]  # shapes: (B, C_in, nrow_i=ncol_i+1)
        row_means = [
            w.mean(dim=-2) for w in weights
        ...
        for i, (weight, bias) in enumerate(wsfeat):
            if i == 0:
                rpt = [
                    rearrange(weight, "b c_in nrow ncol -> b (c_in ncol) nrow"),
                    row_means[1],
                    bias,
                ]
                if i + 1 == self.L - 1:
                    rpt.append(
                        rearrange(
                            weights[-1], "b c_in n_out nrow -> b (c_in n_out) nrow"
                        )
                    )
                ...
                z = rearrange(
                    z1 + z2,
                    "b (c_out ncol) nrow -> b c_out nrow ncol",
                    ncol=weight.shape[-1],
                )
                ...
                cpt = [
                    rearrange(weight, "b c_in nrow ncol -> b (c_in nrow) ncol"),
                    col_means[-2],
                ]  # b c_in ncol
                if i - 1 == 0:
                    cpt.append(
                        rearrange(weights[0], "b c_in ncol n_in -> b (c_in n_in) ncol")
                    )
                z1 = self.wfin_cpt(torch.cat(cpt, dim=-2))  # (B, C_out * nrow, ncol)
                z2 = self.wfin_cbdcst(rc_inp)[..., None]  # (b, c_out * nrow, 1)
                z = rearrange(
                    z1 + z2,
                    "b (c_out nrow) ncol -> b c_out nrow ncol",
                    nrow=weight.shape[-2],
                )
                u = rearrange(
                    self.bias_fin_rc(rc_inp),
                    "b (c_out nrow) -> b c_out nrow",
                    nrow=weight.shape[-2],
                )
            else:
                z1 = getattr(self, f"layer_{i}")(weight)  # (B, C_out, nrow, ncol)
                ...
                if i == 1:
                    row_bdcst.append(
                        rearrange(weights[0], "b c ncol n_in -> b (c n_in) ncol")
                    )
                if i == len(weights) - 2:
                    col_bdcst.append(
                        rearrange(weights[-1], "b c n_out nrow -> b (c n_out) nrow")
                    )
                row_bdcst = torch.cat(
                    row_bdcst, dim=-2
                )  # (B, (3 + ?n_in) * C_in, ncol)
