...
from src.neural_graphs.nn.pooling import HeterogeneousAggregator
...
class GNNForGeneralization(nn.Module):
    def __init__(
        ...
        layers = []
        in_dim = num_graph_features
        # Loop through the multipliers in the config to build hidden layers
        for multiplier in decoder_hidden_dims:
            out_dim = int(multiplier * d_hid)
            layers.append(nn.Linear(in_dim, out_dim))
            layers.append(nn.SiLU())
            in_dim = out_dim # The output of this layer is the input to the next

        # Add the final projection layer to the output dimension
        layers.append(nn.Linear(in_dim, d_out))

        self.proj_out = nn.Sequential(*layers)
